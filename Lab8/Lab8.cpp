...
Student::Student() : univ("SFU")
{
	marks = Array(3);
	for (int i = 0; i<3; ++i)
		marks[i] = 0;
}
Student::Student(const string& name, int age, const string& u, int count) :
	Person(name, age), univ(u)
{
	marks = Array(count);
	for (int i = 0; i<count; ++i)
		marks[i] = 0;
}
Student::Student(const Student& s) : Person(s), univ(s.univ) //îáðàòèòå âíèìàíèå íà âûçîâ êîíñòðóêòîðà ïðåäêà!
{
	marks = Array(s.marks.length());
...
Student& Student::operator=(const Student& s)
{
	if (&s != this)
	{
		Person::operator=(s); //îáðàòèòå âíèìàíèå äëÿ çàêðûòûõ ïîëåé ïðåäêà âûçûâàåì ìåòîä ïðåäêà!
		univ = s.univ;
		marks = Array(s.marks.length());
		for (int i = 0; i<marks.length(); ++i)
			marks[i] = s.marks[i];
